...
class CategoriesDataTable extends DataTable
{
   /**
    ...
    public function dataTable($query)
    {
        ...
        ->addColumn('related', function ($category) {
            $channels = $category->channels()->get()->toArray();
            $channels = array_reduce(array_column($channels, 'channels'), 'array_merge', array());
            $channels = array_unique(array_column($channels, 'id'));
            
            return count($channels);
        })
        ->addColumn('action', function ($category) {
            $action = Auth::user()->can('category.update') ?
            ...
            : 
                "";
            $action .= Auth::user()->can('category.destroy') ?
                '<form class="form-group" action="'. route('dashboard.category.destroy', $category->id) .'" method="POST">
